<div>
    ...
    <form wire:submit="filtrer">
        <div class="row">
            <div class="col-sm-6">
                <span>
                    <b>{{ $personnels->count() }}</b> Résultats sur {{ $total }}.
                </span>
            </div>
            <div class="col-sm-6">
        ...
        <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
            ...
            <tbody>
                @forelse ($personnels as $personnel)
                    <tr>
                        <td>
                            <img src="{{ Storage::url($personnel->photo) }}" width="40 " height="40 "
                                class="rounded shadow" alt="">
                        </td>
                        <td>
                            {{ $personnel->nom }}
                        </td>
                        <td>{{ $personnel->deleted_at->format('d/m/Y') }} </td>
                        <td style="text-align: right;">
                            <button class="btn btn-sm btn-success" type="button"
                                wire:click="restore({{ $personnel->id }})">
                                <i class="bi bi-check-circle"></i>
                                Restorer
                            </button>
                            <button class="btn btn-sm btn-danger" type="button"
                                wire:click="delete_definitif({{ $personnel->id }})">
                                <i class="bi bi-check-circle"></i>
